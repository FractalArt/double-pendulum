...
def parse_args():
    ...
    parser.add_argument('--v1', type=float, default=-np.pi / 9, help='The first initial angular velocity [rad / s]')
    parser.add_argument('--v2', type=float, default=-np.pi / 8, help='The second initial angular velocity [rad / s]')
    ...
    parser.add_argument('--steps', '-s', type=int, default=500_000, help='The number of simulation steps')
...
if __name__ == '__main__':
    ...
    rk = RungeKutta4(args.dt, f, np.array([args.i1, args.i2, 0, 0]))
    ...
    for step in range(args.steps):
        ...
        x1 = args.r1 * np.sin(t1)
        y1 = -args.r1 * np.cos(t1)
        x2 = x1 + args.r2 * np.sin(t2)
        y2 = y1 - args.r2 * np.cos(t2)
